...
class ConstantsManager {
  constructor() {
    this.SUPPORTED_OPERATIONS = [
      ...
      'searchKeyword', 'searchFestival', 'locationBasedList', 'areaCode',
      'categoryCode', 'batchDetail'
    ];
...
class ConfigManager {
  ...
  validateConfig() {
    ...
    if (!this.hasValidApiKey()) {
      errors.push('TOURISM_API_KEY is required');
    }
...
class ApiResponseProcessor {
  static extractItems(apiResponse) {
    try {
      ...
      if (body.items) {
        return Array.isArray(body.items.item) ? body.items.item : (body.items.item ? [body.items.item] : []);
      }
...
class AllTourismAPI {
  ...
  async areaBasedList(params = {}) {
    try {
      ...
      const sanitizedParams = validator.validate('areaBasedList', params);
      const {
      ...
      const cacheableParams = { numOfRows, pageNo, arrange, contentTypeId, areaCode, sigunguCode, cat1, cat2, cat3 };
      ...
      const optionalParams = { contentTypeId, areaCode, sigunguCode, cat1, cat2, cat3 };
      ...
      const logger = this.container.get('logger');
      logger.error('AreaBasedList error', error);
      return ResponseFormatter.formatError(error, 'areaBasedList');
    }
  }
  ...
  async detailCommon(params = {}) {
    try {
      ...
      const sanitizedParams = validator.validate('detailCommon', params);
      const { contentId } = sanitizedParams;
      ...
      const logger = this.container.get('logger');
      logger.error('DetailCommon error', error);
      return ResponseFormatter.formatError(error, 'detailCommon');
    }
  }
  ...
  async searchKeyword(params = {}) {
    try {
      ...
      const sanitizedParams = validator.validate('searchKeyword', params);
      const {
        keyword, numOfRows = '10', pageNo = '1', arrange = 'A',
        contentTypeId = '', areaCode = '', sigunguCode = '',
        userLat = '', userLng = '', radius = ''
      } = sanitizedParams;
      ...
      const logger = this.container.get('logger');
      logger.error('SearchKeyword error', error);
      return ResponseFormatter.formatError(error, 'searchKeyword');
    }
  }
  ...
  getSystemStatus() {
    try {
      const logger = this.container.get('logger');
      const cache = this.container.get('cache');
      const configManager = this.container.get('config');
  ...
  destroy() {
    try {
      ...
      return { success: true, message: 'API instance destroyed successfully' };
    } catch (error) {
      console.error('Error destroying API instance:', error);
